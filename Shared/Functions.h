...
class Functions
{
	public:
		...
		// Convert UTF8 string to wide stringstd::string& _strSource
		...
		// Get random range
		static float getRandomRange(float _fLower, float _fUpper);
		static float getRandomPercent();
		static int getRandomRange(int _iLower, int _iUpper);
		...
		// Ease in
		static float easeIn(float _fTime);

		// Ease out
		static float easeOut(float _fTime);

		// Ease in out
		static float easeInOut(float _fTime);
};
